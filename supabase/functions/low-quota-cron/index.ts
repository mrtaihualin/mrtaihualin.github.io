  ...
  try {
    ...
    for (const s of students) {
      ...
      try {
        ...
        //      พรุ่งนี้คนนั้นไม่มีคาบแล้ว = ไม่มีรอบที่ 2 → ต้องบอกครูทันที ให้ครูไปพูดเอง
        errCount++;
        const whyLQ = (e && e.message) ? e.message : String(e);
        console.error('[low-quota-cron] ⚠️ ส่งข้อความ "คาบสุดท้าย" หานักเรียนไม่สำเร็จ '
          + '(ไม่มีรอบพรุ่งนี้ให้ลองใหม่ เพราะพรุ่งนี้เขาไม่มีคาบแล้ว):', whyLQ, 'token=', s.token);
        if (teacherUserId) {
          try {
            await pushLine(channelToken, teacherUserId,
              '⚠️ 「最後一堂課」的提醒沒送到 ' + (s.name || s.token) + ' 手上（' + whyLQ + '）\n'
              + '這個提醒只會在「有課的當天」發，明天不會再自動重試，麻煩你自己跟他說一聲。');
          } catch (e2) {
            console.error('[low-quota-cron] ⚠️ แจ้งครูว่าส่งไม่สำเร็จ ก็ส่งไม่สำเร็จอีก:', (e2 && e2.message) || e2);
          }
        }
      }
    }
