-- 🛑🛑🛑 ห้ามรันไฟล์นี้อีกแล้ว ทั้งไฟล์ — เพิ่มคำเตือน 2026-07-31 🛑🛑🛑
-- ────────────────────────────────────────────────────────────────────────────
-- ไฟล์นี้ถูกยกเลิกไปแล้วทั้งไฟล์ ทุกอย่างในนี้ถูกย้ายไปรวมที่:
--     supabase/sql/2026-07-30_submit_class_request_consolidated.sql
--     (รวม is_teacher_caller() ไปด้วยแล้ว — ดูคำอธิบายที่บรรทัด 8-9 ของไฟล์นั้น)
--
-- ถ้าเผลอรันไฟล์นี้ซ้ำ จะเกิด 2 อย่างพร้อมกัน:
--   (1) ได้ submit_class_request ซ้อนกันหลายตัว (ไฟล์นี้สร้างตัว 7 ช่องรับค่า กับ 8 ช่องรับค่า
--       ส่วนของจริงตอนนี้เป็นตัว 9 ช่อง) → Postgres เลือกไม่ถูก ตอบว่า
--       "Could not choose the best candidate function" = เว็บส่งคำขออะไรไม่ได้เลยสักอย่าง
--       ทั้งยกเลิก / ขอเลื่อน / ขอเพิ่มคาบ — เป็นเหตุการณ์ที่เกิดขึ้นจริงมาแล้ว 2026-07-30
--   (2) 2 ก้อนในไฟล์นี้ไม่มีด่าน "นักเรียนยกเลิกได้เฉพาะเหลือ 24 ชม.ขึ้นไป"
--       → ด่านนั้นจะหายไปเงียบๆ ไม่มี error ไม่มีใครรู้ตัว
--
-- 👉 เก็บไฟล์นี้ไว้เป็นประวัติเท่านั้น (ไว้ดูว่าฐานข้อมูลเคยเป็นยังไง) ห้ามกด Run
-- ════════════════════════════════════════════════════════════════════════════
