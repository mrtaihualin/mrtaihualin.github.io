-- 🛑🛑🛑 2026-08-07 — ย้ายเข้ากรุแล้ว ทั้งไฟล์ห้ามรัน ห้ามใช้เป็นต้นฉบับอีกต่อไป 🛑🛑🛑
-- ────────────────────────────────────────────────────────────────────────────
-- เหตุผลที่ย้าย (งาน P2-04/P2-06 — Lin อนุมัติ 2026-08-07): ไฟล์นี้เคยปนกัน 3 อย่างในไฟล์เดียว
--   ผิดเงื่อนไขจบ P2 ("SQL ทดสอบและ SQL เปลี่ยนระบบไม่ปะปนกัน") ตรงๆ:
--     1. is_teacher_caller() — ของจริงที่ยังใช้งานอยู่ → 🆕 ย้ายไปอยู่ไฟล์ใหม่แล้ว:
--        sql/2026-08-07_is_teacher_caller.sql (คัดลอกตรงๆ ไม่มีแก้เนื้อหา)
--     2. submit_class_request เวอร์ชัน 9 พารามิเตอร์ — ถูกแทนที่แล้วโดย
--        sql/2026-08-02_rpc_guards_merged.sql (ยืนยันตรงกับ production ผ่าน P2-03)
--     3. ชุดทดสอบ _test_cancel_guard ท้ายไฟล์ — ใช้ไม่ได้แล้วตั้งแต่ 2026-07-31 (ใช้ token ปลอมที่
--        ถูกปฏิเสธตั้งแต่ด่านแรกหลังงาน C14) มีตัวใหม่แทนอยู่แล้วที่
--        sql/เลิกใช้แล้ว_ห้ามรัน/2026-07-31_submit_verify_token_TEST.sql
--
-- ทุกฟังก์ชันที่ยังใช้จริงตอนนี้ ดูสารบัญ: supabase/sql/00_ฟังก์ชันไหนอยู่ไฟล์ไหน.md
-- เนื้อหาด้านล่างทั้งหมดคือของเดิม เก็บไว้อ่านเป็นประวัติเฉยๆ ไม่มีการแก้ไข
-- ════════════════════════════════════════════════════════════════════════════

-- ════════════════════════════════════════════════════════════════════════════
-- 2026-07-30 — รวม submit_class_request ให้เหลือไฟล์เดียว + เพิ่มด่านห้ามนักเรียน "ยกเลิก"
-- ────────────────────────────────────────────────────────────────────────────
-- ⚠️ ไฟล์นี้แทนที่ทั้งหมด — ห้ามรันไฟล์เก่าเหล่านี้ซ้ำอีกต่อไป (จะย้อนกลับสิ่งที่แก้ในไฟล์นี้)
--
-- 🔄 อัปเดต 2026-07-31: ไฟล์เก่าทั้งหมดถูกยกไปเก็บในกรุแล้ว ที่อยู่ใหม่คือ
--    supabase/sql/เลิกใช้แล้ว_ห้ามรัน/
--      - 2026-07-26_submit_class_request_เวอร์ชันเก่า_7และ8ช่อง.sql
--          (แยกออกมาจาก supabase/2026-07-26_student-rpc-add-rate-limit.sql
--           → ไฟล์ต้นทางนั้นตอนนี้เหลือแต่ของที่ยังใช้จริง 8 ตัว รันซ้ำได้ทั้งไฟล์ตามปกติแล้ว)
--      - 2026-07-26_force_initiated_by.sql
--          (ทั้งไฟล์ ถูกรวมเข้ามาที่นี่หมดแล้ว รวม is_teacher_caller() ด้วย)
--      - 2026-07-19_allow_add_class_request_type.sql
--          (เจอเพิ่มตอนตรวจ 2026-07-31 — มี submit_class_request เวอร์ชันเก่าอีก 2 ตัว
--           อันตรายเท่ากันกับ 2 ไฟล์บน แต่เดิมไม่มีใครสังเกตเห็น)
--
-- 📇 สารบัญว่าฟังก์ชันไหนต้นฉบับล่าสุดอยู่ไฟล์ไหน: supabase/sql/00_ฟังก์ชันไหนอยู่ไฟล์ไหน.md
--
...
--
-- 🛑🛑 อัปเดต 2026-07-31 — ไฟล์นี้ไม่ใช่ของล่าสุดแล้ว ห้ามรันซ้ำ 🛑🛑
--    submit_class_request ถูกแก้ต่อในงาน C14 (เพิ่มด่าน "รหัสนักเรียนต้องมีอยู่จริง"
--    + เอาชื่อจากฐานข้อมูลแทนการเชื่อชื่อที่ส่งมา)
--    ต้นฉบับล่าสุดอยู่ที่: supabase/sql/2026-07-31_submit_verify_token.sql
--    รันไฟล์นี้ซ้ำ = ด่านใหม่นั้นหายไปเงียบๆ (ช่องรับค่าเท่าเดิม จึงเขียนทับได้ ไม่มี error เตือน)
--
--    ⚠️ ชุดทดสอบท้ายไฟล์นี้ (_test_cancel_guard) ใช้ token ปลอม 'test-token-does-not-exist'
--       หลังงาน C14 มันจะถูกปฏิเสธตั้งแต่ด่านแรก → ผลลัพธ์จะอ่านเพี้ยน อย่าใช้ชุดนี้อีก
--       ใช้ตัวใหม่แทน: supabase/sql/2026-07-31_submit_verify_token_TEST.sql
--
--    ✅ ส่วนที่ยังใช้ได้จากไฟล์นี้: is_teacher_caller() (ยังเป็นต้นฉบับล่าสุด)
...
-- ────────────────────────────────────────────────────────────────────────────
-- 🛑 ก้อนนี้ "ห้ามรัน" แล้ว (เพิ่มคำเตือน 2026-07-31 หลังงาน C14)
--    ตัวล่าสุดอยู่ที่ supabase/sql/2026-07-31_submit_verify_token.sql
--    รันก้อนนี้ทับ = ด่าน "รหัสนักเรียนต้องมีอยู่จริง" หายไปเงียบๆ ไม่มี error เตือน
--    (ช่องรับค่า 9 ช่องเท่ากัน จึงเขียนทับได้สนิท ไม่มีอะไรฟ้อง)
--    ✅ ส่วน is_teacher_caller() ข้างบน (บรรทัด 51) ยังเป็นของล่าสุด รันได้ปกติ
