-- 🛑🛑🛑 ไฟล์นี้เลิกใช้แล้ว — ห้ามรันเด็ดขาด (เพิ่มคำเตือน 2026-07-31) 🛑🛑🛑
-- ════════════════════════════════════════════════════════════════════════════
-- ในไฟล์นี้มี submit_class_request 2 เวอร์ชันเก่า (แบบ 7 ช่องรับค่า และ 8 ช่องรับค่า)
...
--                        (เวอร์ชัน 9 ช่องรับค่า)
--
-- ถ้าเผลอรันไฟล์นี้ = ฟังก์ชันซ้อนกัน → Postgres ตอบ "Could not choose the best candidate
-- function" → เว็บส่งคำขออะไรไม่ได้เลยสักอย่าง (ยกเลิก/ขอเลื่อน/ขอเพิ่มคาบ พังหมด)
-- และด่าน "นักเรียนยกเลิกได้เฉพาะเหลือ 24 ชม.ขึ้นไป" กับด่าน "ห้ามนักเรียนอ้างว่าเป็นครู"
-- จะหายไปเงียบๆ ทั้งคู่ (2 ก้อนในไฟล์นี้ไม่มีทั้งคู่)
--
-- 👉 เก็บไว้เป็นประวัติเท่านั้น ห้ามกด Run · สารบัญ: supabase/sql/00_ฟังก์ชันไหนอยู่ไฟล์ไหน.md
-- ════════════════════════════════════════════════════════════════════════════
