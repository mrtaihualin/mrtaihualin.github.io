...
-- แล้ว คำสั่ง cron จริงเหลือแค่ `select private.call_calendar_schedule_sync_cron();` ไม่มี apikey/token
-- ฝังให้เห็นในคำสั่งอีกต่อไป — สถานะ ✅ ใช้งานจริงตั้งแต่ 2026-08-07)
-- ไฟล์นี้ (2026-07-17) ยังฝัง anon key ตรงๆ ในคำสั่ง (ปลอดภัย เพราะเป็น anon key ที่เปิดเผยได้ตามกฎ RLS
-- ไม่ใช่ของหลุด) แต่เป็นเอกสารที่ไม่ตรงกับของจริงบน production อีกต่อไป ย้ายเข้าโฟลเดอร์นี้เมื่อ 2026-08-08
