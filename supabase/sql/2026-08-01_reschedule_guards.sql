...
-- ════════════════════════════════════════════════════════════════════════════
--
-- 🔄 อัปเดตรอบ 2 (2026-08-02) — **ถ้ารันไฟล์นี้ไปแล้ว ต้องรันใหม่อีกรอบ**
--    เพิ่มด่านตายตัวใน student_update_own_request: "ห้ามแก้เวลาที่ขอ ตอนครูกำลังจัดการอยู่"
--    (ของเดิมเช็คก็ต่อเมื่อฝั่งเว็บสั่งให้เช็ค = ยิง API ตรงข้ามด่านได้ ดูป้าย ★★★ ข้างล่าง)
--    ปลอดภัยที่จะรันซ้ำ · จำนวนช่องรับค่ายังเท่าเดิมทุกตัว ไม่เกิดฟังก์ชันซ้อน
--
...
-- │ ★★★ รอบ 2 (2026-08-02): เพิ่มด่านตายตัว "ห้ามแก้เวลาที่ขอ ตอนครูกำลังทำอยู่" │
...
begin
  ...
  -- ══════════════════════════════════════════════════════════════════════════
  -- ★★★ ด่านใหม่ รอบ 2 (2026-08-02 — จากการตรวจซ้ำด้วยสายตาใหม่)
  --
  -- 🕳️ รูที่อุด: ด่าน "นักเรียนห้ามแก้เวลาที่ขอ ตอนครูกำลังย้ายคาบอยู่" ที่เพิ่งเพิ่มไปเมื่อวาน
  --    **อยู่แค่ในหน้าเว็บ** — เพราะ p_require_not_processing / p_require_status
  --    เป็นค่าที่ "คนเรียก" ส่งมาเอง → ไม่ส่งมา = ด่านไม่ทำงาน
  --    ใครยิง RPC ตรงโดยไม่ใส่ธง 2 ตัวนี้ ก็แก้เวลาได้ตลอด แม้ครูกำลังคุยกับ Google Calendar อยู่
  --    ผลจริง: ครูกดปุ่มที่เขียนว่า "8/5 14:00" แต่คาบไปลงเวลาที่นักเรียนเพิ่งเปลี่ยน โดยไม่มีใครรู้
  --
  --    ขัดกับกฎที่ไฟล์นี้เขียนไว้เองที่บรรทัด ~118: "ซ่อนปุ่มไม่ใช่การป้องกัน ใครก็ยิง API ตรงได้"
  --    (ตัว respond_to_offer_as_student ข้างล่างในไฟล์นี้ฝังด่านตายตัวไว้แล้ว — ตัวนี้ยังไม่ได้ฝัง)
  --
  -- 🔒 ขอบเขต ตั้งใจแคบที่สุดเท่าที่จะแคบได้ (กันกระทบระบบอื่นที่คนอื่นดูแลอยู่):
  --    · เฉพาะคำขอชนิด 'reschedule' เท่านั้น → 'cancel' / 'add_class' ไม่ถูกแตะแม้แต่นิดเดียว
  --    · เฉพาะตอนที่ patch แตะ "เวลาที่ขอ" จริงๆ (requested_date / requested_time / proposed_options)
  --      → การแก้ status เป็น acknowledged (ถอนคำขอ) ไม่ถูกแตะ ยังใช้ธงเดิมของผู้เรียกเหมือนเดิม
  --    · ครูไม่ถูกแตะเลย
  --
  -- ✅ ตรวจแล้วว่าเส้นทางจริงทั้ง 3 ทางของฝั่งเว็บผ่านด่านนี้ได้ปกติ (classroom/index.html):
  --      6394 เติมตัวเลือกหลังส่งคำขอ (คำขอเพิ่งสร้าง = pending + ล็อกว่าง)
  --      6439 นักเรียนแก้คำขอตัวเอง (ส่งธง notProcessing มาอยู่แล้ว)
  --      7053 นักเรียนเลือกเวลาที่ครูเสนอ (ส่งธง notProcessing มาอยู่แล้ว)
  -- ══════════════════════════════════════════════════════════════════════════
  if not public.is_teacher_caller()
...
-- ⚠️ ยกเว้นช่องเดียว: "ต้องมี_ด่านรอบ2_ห้ามแก้ตอนครูทำอยู่" จะเป็น true เฉพาะแถว
--    student_update_own_request เท่านั้น · แถว submit_class_request เป็น false = ถูกต้องแล้ว
--    (ด่านนั้นอยู่ในฟังก์ชันเดียว ไม่ได้อยู่ทั้งสองตัว)
-- ════════════════════════════════════════════════════════════════════════════
select p.proname                                                        as ฟังก์ชัน,
       ...
       (pg_get_functiondef(p.oid) like '%locked or already closed%')    as ต้องมี_ด่านรอบ2_ห้ามแก้ตอนครูทำอยู่
from pg_proc p
