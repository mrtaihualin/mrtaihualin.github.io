-- ⚠️⚠️ ไฟล์นี้เป็น "เวอร์ชันแรก" ไม่ใช่ของล่าสุดแล้ว — ห้ามรันซ้ำ (เพิ่มคำเตือน 2026-07-31) ⚠️⚠️
-- ────────────────────────────────────────────────────────────────────────────
-- ฟังก์ชัน 2 ตัวในไฟล์นี้ (student_get_own_requests / student_update_own_request)
-- ถูกแก้ต่ออีก 2 รอบหลังจากวันนี้ ต้นฉบับล่าสุดอยู่ที่:
--     supabase/2026-07-26_student-rpc-add-rate-limit.sql
--
-- รันไฟล์นี้ซ้ำจะ "ไม่พังทันที" (ช่องรับค่าเท่าเดิม จึงเขียนทับได้ ไม่เกิดฟังก์ชันซ้อน)
-- แต่จะย้อนของ 2 อย่างหายไปเงียบๆ ไม่มี error ไม่มีใครรู้ตัว:
--   (1) ค่า offer_status = 'declined' ที่เพิ่มเมื่อ 2026-07-20 → ปุ่ม 婉拒 บนเว็บของนักเรียนจะพังทันที
--   (2) ยามเฝ้าประตูกันคนไล่เดา token (20 ครั้ง/60 วิ ต่อ IP) ที่เพิ่มเมื่อ 2026-07-26 → รูโหว่กลับมา
--
-- 👉 อ่านไฟล์นี้เป็น "ประวัติ/คำอธิบายว่าทำไมต้องใช้ RPC" ได้ตามปกติ (โค้ดเว็บอ้างถึงไฟล์นี้อยู่ที่
--    classroom/index.html:5538) แต่ห้ามกด Run · สารบัญ: supabase/sql/00_ฟังก์ชันไหนอยู่ไฟล์ไหน.md
-- ════════════════════════════════════════════════════════════════════════════
