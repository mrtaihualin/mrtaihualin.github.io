-- HIGH RISK: exact rollback for 20260905043843_email_otp_idempotent_resend.sql.
-- Redeploy the immediately previous email-otp-auth Edge version first.
-- Do not run without Lin's exact Production rollback approval.

begin;

do $$
begin
  if pg_catalog.to_regprocedure('public.confirm_email_otp_delivery_internal(uuid,text,text)') is null
     or pg_catalog.to_regprocedure('public.get_email_otp_delivery_status_internal(uuid,text)') is null
     or not exists (
       select 1 from information_schema.columns
        where table_schema = 'private'
          and table_name = 'email_otp_challenges'
          and column_name = 'delivery_confirmed_at'
     ) then
    raise exception 'EMAIL_OTP_IDEMPOTENT_ROLLBACK_PRECHECK_FAILED';
  end if;
end;
$$;
...
drop function public.get_email_otp_delivery_status_internal(uuid, text);
drop function public.confirm_email_otp_delivery_internal(uuid, text, text);
alter table private.email_otp_challenges drop column delivery_confirmed_at;
