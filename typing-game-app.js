...
var MINA_LINES={
  ...
  correct:['哇～打對了呢，你做得很好 ✨','對了對了！就是這樣打 🌾','很好喔～鍵位越來越熟了 😊'],
  combo:['哇～連續打對，米娜都替你開心 🔥','停不下來了呢，好厲害 ⚡'],
...
function setGuideMode(on){
  ...
  [['guide-on','guide-off']].forEach(function(p){
    var a=document.getElementById(p[0]),b=document.getElementById(p[1]);
    if(a)a.className='btn '+(guideMode?'btn-primary':'btn-secondary');
    if(b)b.className='btn '+(guideMode?'btn-secondary':'btn-primary');
  });
  ...
  try{
    ...
    var wkbDiv=document.getElementById('rg-webkbd-divider');
    if(wkb){ wkb.style.display=guideMode?'none':'inline-block'; wkb.textContent='⌨️'; wkb.className='btn btn-secondary'; }
    if(wkbDiv){ wkbDiv.style.display=guideMode?'none':'inline-block'; }
  }catch(e){}
...
function rgToggleWebKbd(){
  ...
  // Lin 2026-07-19: เหลือแค่ไอคอน ⌨️ ไม่มีตัวหนังสือ — โชว์สถานะเปิด/ปิดด้วยสีปุ่มแทน (เหมือนปุ่มไข่/ข้าวปั้นตอนกำลังเล่นเสียง)
  if(b){ b.className='btn '+(open?'btn-primary':'btn-secondary'); b.title=open?'關閉螢幕鍵盤':'開啟螢幕鍵盤'; }
}
