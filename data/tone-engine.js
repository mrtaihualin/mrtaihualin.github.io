...
(function (global) {
  ...
  // ณ เพิ่ม 2026-07-31 ตาม Lin ยืนยัน — หณ เป็นอักษรนำจริง (ออกเสียงเหมือน หน เช่น ขณะ อ่าน ขะ-หนะ)
  var SONORANT_AFTER_H = new Set(['น', 'ม', 'ล', 'ว', 'ย', 'ร', 'ญ', 'ง', 'ณ']);
  ...
  // ── แก้บั๊ก (e) — เพิ่ม 2026-07-31: คำยืมบาลี/สันสกฤตบางคำ เขียนท้ายคำ "เกิน" กว่าเสียงที่พูดจริง
  // (เช่น ญาติ เขียนลงท้าย ติ แต่พูดจริงแค่เสียง ต / บัตร เขียนลงท้าย ตร แต่พูดจริงแค่เสียง ต)
  // ตัวอักษรส่วนเกิน (ิ ในญาติ / ร ในบัตร) ไม่ออกเสียง แต่เครื่องคิดเดิมยังนับเป็นสระ/ตัวสะกดจริงอยู่ →
  // ทำให้คำนวณสระสั้น-ยาวหรือตาย-เป็นผิด (ญาติ ได้ 短母音 ที่ผิด ที่ถูกคือ 長母音 เพราะ ิ ไม่ออกเสียง)
  // ⚠️ ต้องเจาะจงเป็น "คำ" เท่านั้น ห้ามใช้กฎ pattern ทั่วไป (เช่น "ลงท้าย ติ = ตัดทิ้งเสมอ") เพราะภาษาไทย
  // มีคำปกติจำนวนมากที่ลงท้าย ติ แล้วออกเสียงจริง (เช่น สติ อ่าน สะ-ติ ไม่ใช่ สะ-ต) — ใส่เพิ่มได้ทีละคำ
  // เฉพาะคำที่ตรวจสอบแล้วจริง (มี finalDisp ในข้อมูลคำนั้นยืนยันอยู่แล้ว) ห้ามเดาเติมเอง
  // เพิ่ม 2026-07-31: 'หมัคร' (พยางค์ 2 ของ สมัคร ต่อ lead ห กลับแล้ว) — ร ท้ายเป็นเสียงเกินไม่ออกเสียงจริง
  // (มี finalDisp:'คร' ยืนยันอยู่แล้วในข้อมูลคำนี้ ตามกฎ "ต้องมี finalDisp ยืนยันก่อนถึงเพิ่มได้")
  // เพิ่ม 2026-08-01c: 'จันทร์' (พยางค์ 2 ของ วันจันทร์) — ทร์ เป็นตัวการันต์ 2 ตัวติดกัน (ท+ร์) ที่ทั้งคู่
  // ไม่ออกเสียงเลย เหลือแค่ "จัน" (final:'น' ยืนยันอยู่แล้วในข้อมูลคำนี้) เดิม _stripKarun ตัดได้แค่ตัวอักษร
  // เดียวหน้าการันต์ (ร) ทำให้เหลือ "จันท" ค้าง กลายเป็นคำตาย(ท ปิดท้าย)คำนวณผิดเป็นเอก ทั้งที่เสียงจริง "จัน"
  // เป็นคำเป็น(น ปิดท้าย)=สามัญ — Lin ยืนยันจากเสียงจริง "วันจัน" 2026-08-01c
  var TRAILING_SILENT_FIX = { 'ญาติ': 'ญาต', 'บัตร': 'บัต', 'หมัคร': 'หมัก', 'จันทร์': 'จัน' };

  // ── แก้บั๊ก (h) — เพิ่ม 2026-07-31: บางพยางค์เขียนพยัญชนะตัวเดียวโดดๆ ไม่มีสระกำกับเลย (เช่น "บ" ใน
  // บริษัท พูดจริง "บอ" สระ ออ ยาว) เครื่องคิดเดิมหาสระในตัวหนังสือไม่เจอเลย → เดาผิด ทั้งที่ฟิลด์
  // syl.vowel ของ Lin บอกไว้ถูกต้องอยู่แล้วว่าเป็นสระอะไร — ตารางนี้แปลง "ชื่อสระ" (syl.vowel) กลับเป็น
  // "ตัวสัญลักษณ์สระที่เขียนต่อท้ายพยัญชนะ" (ก็อปมาจาก VOWEL_SYMBOL ในreading-game-app.js ตารางเดียวกัน)
  // ใช้เติมสระที่ขาดหายไปกลับเข้าไปก่อนส่งวิเคราะห์ ไม่กระทบตัวสะกดจริง (th) ที่โชว์บนหน้าจอเลย
  // ⚠️ จำกัดขอบเขตแค่ th ยาว "1 ตัวอักษรเป๊ะ" (พยัญชนะล้วนๆ ไม่มีอะไรอื่นเลย) เท่านั้น — ห้ามใช้เช็คแบบ
  // "หาสระในตัวหนังสือไม่เจอ" แบบกว้างๆ เพราะ ว บางทีก็ทำหน้าที่แทนสระ (พวก/ดวง/รวม ตามแก้บั๊ก (f))
  // ถ้าเช็คกว้างไปจะไปเข้าใจผิดคำพวกนั้นว่า "ไม่มีสระ" แล้วยัดสระซ้อนเข้าไปอีกชั้นจนพัง (เจอจริงตอนทดสอบ
  // ก่อนใช้จริง — พวก/ตลาด/ขวด ฯลฯ พังหมดตอนแรกที่ลองใช้เงื่อนไขกว้าง จึงจำกัดเหลือแค่กรณี 1 ตัวอักษรเป๊ะ)
  var VOWEL_SYMBOL_FOR_INJECT = {
    'อะ':'ะ','อา':'า','ออ':'อ',
    'เอาะ':'เาะ','เออะ':'เะ',
  ...
  // ── แก้บั๊ก (b) ตัวการันต์: ตัดพยัญชนะหน้า ์ + ์ เอง ออกก่อนวิเคราะห์ทั้งหมด ──
  ...
  function _normalize(word) {
    word = TRAILING_SILENT_FIX[word] || word;
  ...
  // ── ใช้โดยแก้บั๊ก (g): คืน "รายการพยัญชนะสำหรับหาตัวสะกด" — ถ้าคำมีอักษรนำ (ห/อย) และไม่มีพยัญชนะ
  // อยู่หลังอักษรนำเลยสักตัว (แปลว่าตัวอักษรนำ 2 ตัวคือพยัญชนะทั้งหมดของพยางค์ ไม่มีตัวสะกดจริง) → คืน []
  // ไปเลย กันไม่ให้ตัวอักษรนำตัวที่ 2 (เช่น ณ ใน หณะ) ถูกเข้าใจผิดว่าเป็นตัวสะกด — ถ้ามีพยัญชนะจริงอยู่
  // หลังอักษรนำ (เช่น ก ใน หนัก) ให้ทำงานแบบเดิมทุกอย่าง ไม่ตัดอะไรออก (กันพังคำที่มีตัวสะกดจริงอยู่แล้ว)
  function _codaConsForFinal(word) {
    var cons = _codaCons(word);
    var ll = _leadLen(word);
    if (ll && cons.length >= 2) {
      var hasConsAfterLead = false;
      for (var i = ll; i < word.length; i++) { if (CONS_SET.has(word[i])) { hasConsAfterLead = true; break; } }
      if (!hasConsAfterLead) cons = [];
    }
    return cons;
  }
  ...
  var ENGINE = {
    ...
    isLiveWord: function (word) {
      word = _normalize(word);
      // ── แก้บั๊ก (g) — เพิ่ม 2026-07-31: คำที่มีอักษรนำ (ห/อ + พยัญชนะ) แล้ว "ไม่มีตัวสะกดจริงต่อท้าย"
      // (เช่น ณะ ในขณะที่ → เต็มรูปเป็น "หณะ") เดิม _codaCons ไล่เก็บพยัญชนะทุกตัวในคำรวมอักษรนำเข้าไปด้วย
      // แล้วเข้าใจผิดว่าตัวอักษรนำตัวที่ 2 (ณ) เป็นตัวสะกด → ฟันธงว่าเป็นเสียงเป็นทั้งที่จริงไม่มีตัวสะกด
      // (เป็นเสียงตาย) แก้โดยใช้ _codaConsForFinal (ดูคอมเมนต์ตรงนิยามฟังก์ชัน) แทน _codaCons ตรงๆ
      var cons = _codaConsForFinal(word);
      if (cons.length >= 2) {
    ...
    getVowelType: function (word) {
      ...
      // ── แก้บั๊ก (f) — เพิ่ม 2026-07-31: สระ "อัว" แบบมีตัวสะกด เขียนแบบไม่มีไม้ ั (ตัดรูปเป็น "ว" ลอย
      // เช่น พวก, ดวง, รวม, สวย — เทียบ อัว ปกติที่เขียน ั+ว) เจอจาก "พวก" คำนวณผิดเป็นสระสั้น ที่ถูกคือยาว
      // (เพราะ ว ตัวนี้ไม่ใช่ตัวอักษรในชุด SHORT_V/LONG_V เลย เครื่องคิดเลยหาสระไม่เจอเลย → ตกไปเป็น null →
      // นับผิดเป็นสั้นไปโดยปริยาย) กฎ: ถ้าสแกนทั้งคำไม่เจอสระตัวไหนเลย (มาถึงบรรทัดนี้ได้) และคำมีรูปแบบ
      // "พยัญชนะ+ว+พยัญชนะ" (ว แทรกกลางระหว่างพยัญชนะ 2 ตัว ไม่ใช่ตัวแรก/ตัวสุดท้ายของคำ) → ถือว่าเป็น
      // อัว ลดรูปนี้ = ยาว เสมอ (ไม่ชนกับคำที่มี ว เป็นพยัญชนะควบกล้ำจริง เช่น ควาย/ขวาง/ควัน เพราะคำพวกนั้น
      // มีสระตัวอื่นเขียนกำกับอยู่แล้วเสมอ ฟังก์ชันจะเจอสระนั้นแล้ว return ไปตั้งแต่ 2 บรรทัดบน ไม่มาถึงจุดนี้)
      for (var k = 1; k < word.length - 1; k++) {
        if (word[k] === 'ว' && CONS_SET.has(word[k - 1]) && CONS_SET.has(word[k + 1])) return 'long';
      }
      return null;
    },
    ...
    hasTailCons: function (word) {
      word = _normalize(word);
      return _codaConsForFinal(word).length >= 2;
    },
  ...
  function getFullSyllableSpelling(syl) {
    ...
    var th = syl.th;
    if (syl.lead && th.charAt(0) !== 'ห' && th.charAt(0) !== 'อ') th = syl.lead + th;
    // แก้บั๊ก (h): th ต้นฉบับยาว "1 ตัวอักษรเป๊ะ" (พยัญชนะโดดๆ เช่น th:'บ' ไม่มีสระเขียนกำกับเลย)
    // แต่ syl.vowel บอกไว้ถูกแล้วว่าเป็นสระอะไร → เติมสัญลักษณ์สระกลับเข้าไปก่อนวิเคราะห์ (ไม่กระทบ
    // syl.th ต้นฉบับที่ใช้โชว์ผลจริงบนหน้าจอ) — เช็คจาก syl.th.length ตรงๆ (ก่อนต่อ lead) เพื่อไม่ให้
    // ไปชนกับคำที่ ว ทำหน้าที่แทนสระ (พวก/ดวง ฯลฯ) ซึ่งยาวมากกว่า 1 ตัวอักษรอยู่แล้วเสมอ
    if (syl.vowel && syl.th.length === 1 && VOWEL_SYMBOL_FOR_INJECT[syl.vowel]) {
      th = th + VOWEL_SYMBOL_FOR_INJECT[syl.vowel];
    }
    return th;
  }
