...
assert.match(migration, /join _paid_queue_193 p[\s\S]+p\.word=g\.word and p\.zh=g\.zh and p\.level=g\.level[\s\S]+g\.status in \('active','queued'\)/);
assert.doesNotMatch(migration, /join _paid_queue_193 p using\(word\)/);
assert.match(migration, /insert into public\.game_words\([\s\S]+content_key,word,en,zh,level,category,reading_th,rank,spelling_th,audio_status,type,subcategory,[\s\S]+access_tier,catalog_version,record_hash,canonical_record,status,updated_at/);
assert.doesNotMatch(migration, /insert into public\.game_words\([\s\S]*?\bsyls\b/);
assert.doesNotMatch(migration, /insert into public\.game_words\([\s\S]*?\bread_syls\b/);
