...
expect(/#rg-profile-wrap \{[\s\S]*background: transparent !important;[\s\S]*border: 0 !important/.test(loginCss)&&/\.sa-account-bar \{[\s\S]*border: 0 !important/.test(loginCss)&&/\.sa-global-search-input \{[\s\S]*border: 0;/.test(loginCss), 'expanded Account Search has no nested border frame');
