...
test('Five games expose Login Free with owner-safe account runtimes while Paid and Lego account runtime remain parked', async () => {
  ...
  for (const page of corePages) {
    ...
    assert.match(html, /reading-auth\.js\?v=32/, page + ' Login Free account runtime');
  ...
  assert.match(reading, /reading-auth\.js\?v=32/, 'reading Login Free account runtime');
  ...
  for (const page of ['vault.html']) {
    assert.match(read(page), /reading-auth\.js\?v=32/, page + ' reading-auth cache');
  }
