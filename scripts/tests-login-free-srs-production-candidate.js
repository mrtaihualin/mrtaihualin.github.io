...
pages.forEach(function (file) {
  var html = read(file);
  check(html.indexOf('js/core/minimum-guest-launch.js?v=17') !== -1, file + ' fetches the current Login Free launch gate');
...
check(read('js/core/minimum-guest-launch.js').indexOf('login-surface.js?v=12') !== -1, 'SRS pages fetch the account-aware Login surface');
check(read('js/core/login-surface.js').indexOf('reading-auth.js?v=33') !== -1, 'Login surface fetches the account-aware auth client');
check(read('reading-game.html').indexOf('reading-auth.js?v=33') !== -1, 'Reading direct provider flow fetches the account-aware auth client');
