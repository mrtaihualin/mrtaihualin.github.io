...
pages.forEach(function (file) {
  var html = read(file);
  check(html.indexOf('js/core/minimum-guest-launch.js?v=9') !== -1, file + ' fetches the SRS-enabled launch gate with a fresh cache key');
  check(html.indexOf('js/games/tone-server.js?v=5') !== -1, file + ' loads the authenticated SRS transport');
...
check(read('tone-finder.html').indexOf('tone-finder-game.min.js?v=81') !== -1, 'Tone fetches the SRS-enabled game runtime');
check(read('reading-game.html').indexOf('reading-game-app.min.js?v=47') !== -1, 'Reading fetches the SRS-enabled game runtime');
check(read('typing-game.html').indexOf('typing-game-app.min.js?v=45') !== -1, 'Typing fetches the SRS-enabled game runtime');
check(read('word-order.html').indexOf('word-order-app.min.js?v=33') !== -1, 'Word Order fetches the SRS-enabled game runtime');
