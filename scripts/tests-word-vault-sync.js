...
// ════════════════════════════════════════════════════════════════════════════
// 6) 🔴 รวมคำ 2 เครื่องแล้วเกินเพดาน Login Free 50 — ห้ามตัดคำทิ้ง ให้บล็อกการเพิ่มใหม่แทน
// ════════════════════════════════════════════════════════════════════════════
{
  const { vault } = loadVault();
  for (let i = 0; i < 50; i++) vault.addWord('คำA' + i, { zh: 'A' + i });   // เครื่องนี้เต็ม 50
  const remote = [];
  for (let i = 0; i < 25; i++) remote.push({ word_th: 'คำB' + i, zh: 'B' + i, tags: [] }); // เครื่องอื่นอีก 25
  const { client } = makeClient(remote);
  vault.sync(client, UID);
  ok('6a รวมคำได้ครบทั้ง 75 คำ แม้เกินเพดาน 50', vault.count() === 75, 'ได้ ' + vault.count());
  ok('6b ไม่มีคำไหนถูกตัดทิ้ง (เครื่องนี้ครบ 50)',
     vault.has('คำA0') && vault.has('คำA49'));
  ...
  ok('6e บล็อกการเพิ่มคำใหม่ชั่วคราว (ไม่ใช่ตัดคำเก่าทิ้ง)',
     added === false && vault.count() === 75 && !vault.has('คำใหม่'));
  vault.removeWord('คำA0');
  ok('6f ผู้ใช้ลบเองได้ตามปกติ (ทางออกจากสถานะเกินเพดาน)', vault.count() === 74 && !vault.has('คำA0'));
}
