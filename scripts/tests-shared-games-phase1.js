...
test('all scoped pages use one fail-closed Login surface and game pages permanently remove announcements', () => {
  ...
  for (const file of gamePages) {
    ...
    assert.match(html, /minimum-guest-launch\.js\?v=17/, `${file}: must load the current Reading-authority Login gate`);
  ...
  assert.match(minimumGuest, /loginController\.src = 'js\/core\/login-surface\.js\?v=12'/, 'public pages must request the account-aware Login controller');
