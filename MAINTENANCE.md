...
- Locked one top band across all six games: smallest Login slot, Chinese `等級／工具`, centered game-owned title and right-side Game/More/fixed Skip geometry. Both three-control groups use the same responsive `8–12px` breathing room. Word Order and Lego omit Level without a blank gap. Dropdowns and utility menus share below-trigger positioning, icon-and-label rows, six-row scrolling and bounded panels.
