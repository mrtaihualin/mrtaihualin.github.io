...
- Preserved-feature and retired-path behavioral regressions pass, including student/teacher card rendering, both LINE contact states, add-class lock behavior and Calendar reliability. A baseline-to-candidate add-class LINE payload comparison confirms identical delivery fields and Flex rows/buttons; only the obsolete section label in the email fallback message changed. `node scripts/check-site.js` passes all 1,113 local project files; `git diff --check` passes. Source baseline and recovery instructions are archived at `/Users/taihualin/Documents/Claude/Projects/04_WORKING/CLASSROOM_SYSTEM/07_ARCHIVE/CANCEL_RESCHEDULE_RECOVERY_BASELINE_2026-09-19/`. Live Edge/cron, RPC and website deployment, plus Lin's exact Human UI check, remain separate gates.
