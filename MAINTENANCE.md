...
- Independent pre-activation review found four blockers: post-write error returns could preserve partial state, the predecessor event RPC remained callable, rollback could issue rounds without a compatible completion path, and commit day used Bangkok while issuance/refill used Taipei. The repair makes every post-write rejection raise and roll back the statement, removes the service fallback and revokes predecessor/internal helper execution, uses the canonical Taipei day, and makes emergency rollback fully fail-closed for every issue/write RPC while retaining read/export evidence. Exact-operation replay now converges from the owner-bound event log without reopening the predecessor writer.
- Recovery is non-destructive and fail-closed: it requires zero active Typing rounds and revokes atomic, predecessor, issuance and refill writers while preserving all additive round/Retry/learning/score/export evidence. The current protected load and owner-bound export remain available for recovery inspection; forward reapply restores only the single atomic writer. Browser roles retain no table or function write/execute path.
- Verification: atomic final preparation `6/6`, source/security boundary `6/6`, Resume/retry reducer `16/16`, initial plan `18/18`, account export `8/8`, client `13/13`, Edge bridge `14/14`, client/service composition `8/8`, issuance/context/refill `31/31`, disposable PostgreSQL success/replay/Retry/final score/export plus stale-CAS, premature-final, score-conflict, predecessor/helper ACL, Taipei midnight boundary and fail-closed rollback PASS. Prior persistence PostgreSQL `5/5` and atomic continuation PostgreSQL `6/6` regressions PASS after their fixtures were aligned with the new no-fallback owner. `node scripts/check-site.js` passes all checks across `1,168` files. The PostgreSQL/full gates ran outside the filesystem sandbox only to allow disposable shared memory; no shared-memory ID was deleted.
