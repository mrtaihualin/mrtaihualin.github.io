 ...
 *      (Edge Function tone-round ยัง whitelist ไม่รู้จัก 'challenge' — รอ Lin deploy เอง ตอนนี้เซิร์ฟเวอร์ตอบ bad game เงียบๆ) [2026-08-01: เปลี่ยนชื่อ id จาก 'mix' เป็น 'challenge']
 * ════════════════════════════════════════════════════════════════════════════
 *
...
(function () {
  ...
  function markStageDone(th, clean) {
    ...
    if (rec.n >= 3 && window.TONE_SERVER && window.TONE_SERVER.available && window.TONE_SERVER.available()) {
      try { window.TONE_SERVER.finishRound({ word: th, level: 1, game: 'challenge', clean: !rec.anyWrong }); } catch (e) {}
    }
  }
  ...
  function syncWordControls(w) {
    ...
    if (vslot && window.WordVault) {
      ...
      vslot.innerHTML = '';
      vslot.appendChild(WordVault.createSaveBtn(w.th, { zh: w.zh, source: 'games-challenge' }, {}));
    }
  }
  ...
  function renderEnd() {
    ...
    if (window.READING_AUTH && window.READING_AUTH.saveScore) { try { READING_AUTH.saveScore(S.score, 1, 'challenge', wrongItemsForSave()); } catch (e) {} }
    mxRenderGameBar();
